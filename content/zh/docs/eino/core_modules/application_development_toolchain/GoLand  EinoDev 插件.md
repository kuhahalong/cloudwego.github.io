...
- 3 - 历史编排 Graph 列表
- 4 - 新建 Graph 编排任务
- 5 - 将编排完整的 Graph 指定目录生成Eino Graph 代码
- 6、7 - 导入、导出Graph 配置Schema 信息
...
- 完成添加  Node  操作后，可按需配置组件的Runtime Config 信息。
```
...
- 比如官方提供的 volc_vikingDB  组件，其依赖了 Embeding Component 作为插槽；再比如官方提供的ToolsNode 组件，其依赖了多个 Tool  Component。
```
...
```
- 界面左下角新增 Graph，在弹窗对话框填写 Graph 相关配置，生成Graph 画布。
	- 图
- 按需从 AddNodes  选择合适的Node 组件，添加的画布。
	-  图
...
```
- 特别的当添加的 Component 为 Graph 类型时，添加的 嵌套Graph 可展开做Node 组件的配置，配置完成后，通过顶层面包屑路径跳回首页界面。
```
